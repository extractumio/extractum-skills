...
`/code-review-cc → scope the diff → per-angle Find → Verify → (Sweep) → ranked findings`
...
- **`--comment`** — post findings as inline PR comments (see the *Posting to
  GitHub* appendix).
- **`<target>`** — anything left over (a PR number, branch name, or file path)
  is the review target. If a target is given, prepend `Review target:
  \`<target>\`` to the run and review that instead of the default diff range.

Follow the single section below that matches the selected effort level.
...
# Appendix — Posting to GitHub (`--comment`)
